...
data_dict <- data.frame(row1 = c("Variable", "Group", "Subject","Rew","Block","Phase","Opt"),
                        row2 = c("Description",
...
kable(data_dict, row.names = F) %>% column_spec (1:2, border_left = T, border_right = T) %>% kable_styling()
                 
############################ SETTING UP SHOP, READING IN DATA #################################

##Read in data
df = read.table("/Users/mvf327/Library/CloudStorage/Box-Box/Home/Data_Science/_github/Prism_adaptation_and_reward/SS_LME.csv", header = TRUE, sep = ",")
...
Logit_model1 <- lmer(logit_Opt ~ Block_rev_c*Group*Rew*Phase + I(Block_rev_c^2)*Group*Rew*Phase + (1 + (Block_rev_c)+ I(Block_rev_c^2) | Subject), data = df)
Logit_model2 <- lmer(logit_Opt ~ Block_rev_c*Group*Rew*Phase + I(Block_rev_c^2)*Group*Rew*Phase + (1 + (Block_rev_c) | Subject), data = df)
Logit_model3 <- lmer(logit_Opt ~ Block_rev_c*Group*Rew*Phase + I(Block_rev_c^2)*Group*Rew*Phase + (1 | Subject), data = df)
...
write.table(plotdata, "/Users/mvf327/Library/CloudStorage/Box-Box/Home/Data_Science/_github/Prism_adaptation_and_reward/SS_Data.txt", sep="\t")
...
LPA_model <- lmer(logit_Opt ~ Block_rev_c*Rew*Phase+ I(Block_rev_c^2)*Rew*Phase + (1 + (Block_rev_c)+ I(Block_rev_c^2) |Subject), data = df_LPA)
summary(LPA_model)
...
# Rerun logit model 1 with only LPA group
RPA_model <- lmer(logit_Opt ~ Block_rev_c*Rew*Phase+ I(Block_rev_c^2)*Rew*Phase + (1 + (Block_rev_c)+ I(Block_rev_c^2) |Subject), data = df_RPA)
summary(RPA_model)
